...
fuente = pygame.font.Font("proyecto/fonts/GeneralSans-SemiBold.ttf", 15)
flecha = pygame.image.load("proyecto/assets/flecha.png").convert_alpha()
imagen = pygame.image.load("proyecto/assets/ruleta.png").convert_alpha()

# BOTON DE GIRO
btn_giro = Boton(799, 433, 175, 36, "GIRAR / PARAR", value=True, type="bool")
    ...
    for event in pygame.event.get():
        ...
        for btn in btn_cant.get():
            if btn.isclick(event):
                if btn.value == "DEL":
                    cuadrotxt.memoria.pop()
                else:
                    cuadrotxt.memoria.append(int(btn.value))
                    
                cuadrotxt.txt = str(sum(cuadrotxt.memoria))
